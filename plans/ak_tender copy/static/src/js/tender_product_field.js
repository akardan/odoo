...
export class TenderLineProductField extends ProductLabelSectionAndNoteField {
    static template = "ak_tender.TenderProductField";
    setup() {
        super.setup();
    ...
    async _openGridConfigurator(edit) {
        const TenderRecord = this.props.record.model.root;

        // fetch matrix information from server;
        await TenderRecord.update({
        ...
        if (!edit) {
            ...
            TenderRecord.data.tender_lines.delete(this.props.record);
        }
    }
...
export const tenderLineProductField = {
    ...many2OneField,
    component: TenderLineProductField,
};

registry.category("fields").add("tender_product_many2one", tenderLineProductField);