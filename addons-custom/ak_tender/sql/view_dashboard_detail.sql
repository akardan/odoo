...
SELECT
    ...
    (tl.target_price * (1 - tl.target_discount/100) * tl.quantity * COALESCE(1.0 / line_latest_rate.rate, 1.0)) as target_total_untaxed_tl,
    
    -- ============================================================================
    -- İLK TUR FİYATLAR (Round 1)
    -- Birim fiyat: price_unit * (1 - discount/100)
    -- Toplam fiyat: price_subtotal (doğrudan kullan, çünkü = price_unit * (1 - discount/100) * pol2.quantity)
    -- ============================================================================
    ...
    COALESCE((
        SELECT MIN(pol2.price_unit * (1 - COALESCE(pol2.discount, 0) / 100.0) *
            CASE WHEN po_curr.name = 'TRY' THEN 1.0 ELSE COALESCE(1.0 / cr.rate, 1.0) END)
        FROM purchase_order po2
        JOIN purchase_order_line pol2 ON pol2.order_id = po2.id
    ...
    -- İlk Tur Toplam Fiyat (price_subtotal kullan - FIX)
    COALESCE((
    ...
    -- Mevcut Tur Toplam Fiyat (price_subtotal kullan - FIX)
    COALESCE((
        SELECT MIN(pol2.price_subtotal)
        FROM purchase_order po2
        JOIN purchase_order_line pol2 ON pol2.order_id = po2.id
...
FROM ak_tender t
    ...
    -- TRY için kur 1.0, diğer para birimleri için tabloya bak
    LEFT JOIN LATERAL (
        SELECT
            CASE WHEN curr.name = 'TRY' THEN 1.0 ELSE rate END as rate,
            name
        FROM res_currency_rate
    ...
    LEFT JOIN LATERAL (
        SELECT
            CASE WHEN line_curr.name = 'TRY' THEN 1.0 ELSE rate END as rate,
            name
        FROM res_currency_rate
